...
/// <summary>
/// Represents a normalized numeric theme with an optional CSS length unit. Input is validated and canonicalized (e.g.,
/// <c>"12px"</c>, <c>"-3.5 rem"</c>, <c>"100%"</c>, <c>"42"</c>). The canonical string is exposed via
/// <see cref="ThemeBase.Value" />, while <see cref="Number" /> provides the parsed <see cref="double" /> and
...
/// </remarks>
public sealed class ThemeNumber : ThemeBase
{
    /// <summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ThemeNumber" /> class by parsing and normalizing the provided input
    ...
    /// </exception>
    public ThemeNumber(string value)
    ...
    /// <summary>Gets the parsed numeric theme as a <see cref="double" /> using invariant-culture semantics.</summary>
    ...
    /// </param>
    /// <param name="number">
    /// Outputs the parsed numeric theme as a <see cref="double" />. When the input is <c>"auto"</c>, this is set to <c>0</c>.
    /// </param>
    /// <param name="unit">
    /// Outputs the parsed <see cref="LengthUnits" /> theme, or <c>null</c> if the input is unitless or <c>"auto"</c>.
    /// </param>
    /// <returns>
    ...
    ///     <item>
    ///         <description>
    ///         Otherwise, the normalized numeric theme followed immediately by the canonical unit token (or nothing if
    ...
    private static string Normalize(string raw, out double number, out LengthUnits? unit)
    {
        ...
        {
            throw new AryArgumentException("Number is not a finite theme.", nameof(raw), raw);
        }
    ...
    /// <summary>Parses the specified string into an <see cref="ThemeNumber" />.</summary>
    /// <param name="value">The input string to parse.</param>
    /// <returns>A new <see cref="ThemeNumber" /> instance.</returns>
    /// <exception cref="AryArgumentException">Thrown when <paramref name="value" /> cannot be parsed/validated.</exception>
    public static ThemeNumber Parse(string value) => new(value);
    ...
    /// <summary>Attempts to parse the specified string into an <see cref="ThemeNumber" />.</summary>
    /// <param name="value">The input string to parse.</param>
    /// <param name="result">
    /// When this method returns, contains the parsed <see cref="ThemeNumber" /> if parsing succeeded; otherwise <c>null</c> .
    /// </param>
    /// <returns><c>true</c> if parsing succeeded; otherwise, <c>false</c>.</returns>
    public static bool TryParse(string value, out ThemeNumber? result)
    {
        try
        {
            result = new ThemeNumber(value);
    ...
    /// <summary>Defines an implicit conversion from <see cref="string" /> to <see cref="ThemeNumber" />.</summary>
    ...
    public static implicit operator ThemeNumber(string value) => new(value);

    /// <summary>
    /// Defines an implicit conversion from <see cref="ThemeNumber" /> to <see cref="string" />, returning the canonical
    /// <see cref="ThemeBase.Value" /> representation (e.g., <c>"12px"</c>, <c>"100%"</c>, <c>"-3.5"</c>).
    /// </summary>
    /// <param name="theme">The instance to convert.</param>
    /// <returns>The canonical string form.</returns>
    public static implicit operator string(ThemeNumber theme) => theme.Value;
}
