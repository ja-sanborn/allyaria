using Allyaria.Theming.Constants;
using Allyaria.Theming.Enumerations;

namespace Allyaria.Theming.Styles;

/// <summary>
/// Represents a theming variant that contains separate <see cref="AllyariaPalette" /> definitions for light, dark, and
/// high-contrast modes, each with their own elevation hierarchies.
/// </summary>
/// <remarks>
/// This struct enables components to adapt automatically to system or user-selected theme types while maintaining
/// consistent color relationships across different elevations and states.
/// </remarks>
internal readonly record struct AllyariaPaletteVariant
{
    /// <summary>
    /// Initializes a new instance of the <see cref="AllyariaPaletteVariant" /> struct, defining base palettes for light, dark,
    /// and high-contrast modes.
    /// </summary>
    /// <param name="paletteLight">
    /// Optional custom light mode palette. If <see langword="null" />, defaults to
    /// <see cref="StyleDefaults.BackgroundColorLight" /> and <see cref="StyleDefaults.BackgroundColorDark" />.
    /// </param>
    /// <param name="paletteDark">
    /// Optional custom dark mode palette. If <see langword="null" />, defaults to
    /// <see cref="StyleDefaults.BackgroundColorDark" /> and <see cref="StyleDefaults.ForegroundColorDark" />.
    /// </param>
    /// <param name="paletteHighContrast">
    /// Optional custom high-contrast palette. If <see langword="null" />, defaults to
    /// <see cref="StyleDefaults.BackgroundColorHighContrast" /> and <see cref="StyleDefaults.ForegroundColorHighContrast" />.
    /// </param>
    /// <remarks>
    /// Each palette initializes a corresponding <see cref="AllyariaPaletteElevation" /> hierarchy to support consistent
    /// appearance across elevation layers and component states.
    /// </remarks>
    public AllyariaPaletteVariant(AllyariaPalette? paletteLight = null,
        AllyariaPalette? paletteDark = null,
        AllyariaPalette? paletteHighContrast = null)
    {
        LightPalette = paletteLight ?? new AllyariaPalette(
            StyleDefaults.BackgroundColorLight, StyleDefaults.BackgroundColorDark
        );

        DarkPalette = paletteDark ?? new AllyariaPalette(
            StyleDefaults.BackgroundColorDark, StyleDefaults.ForegroundColorDark
        );

        HighContrastPalette = paletteHighContrast ?? new AllyariaPalette(
            StyleDefaults.BackgroundColorHighContrast, StyleDefaults.ForegroundColorHighContrast
        );

        LightElevation = new AllyariaPaletteElevation(LightPalette);
        DarkElevation = new AllyariaPaletteElevation(DarkPalette);
        HighContrastElevation = new AllyariaPaletteElevation(HighContrastPalette);
    }

    /// <summary>Gets the elevation hierarchy for dark mode.</summary>
    internal AllyariaPaletteElevation DarkElevation { get; }

    /// <summary>Gets the base dark mode palette.</summary>
    internal AllyariaPalette DarkPalette { get; }

    /// <summary>Gets the elevation hierarchy for high-contrast mode.</summary>
    internal AllyariaPaletteElevation HighContrastElevation { get; }

    /// <summary>Gets the base high-contrast mode palette.</summary>
    internal AllyariaPalette HighContrastPalette { get; }

    /// <summary>Gets the elevation hierarchy for light mode.</summary>
    internal AllyariaPaletteElevation LightElevation { get; }

    /// <summary>Gets the base light mode palette.</summary>
    internal AllyariaPalette LightPalette { get; }

    /// <summary>
    /// Returns a new <see cref="AllyariaPaletteVariant" /> with optional overrides for one or more theme palettes.
    /// </summary>
    /// <param name="paletteLight">Optional override for the light mode palette.</param>
    /// <param name="paletteDark">Optional override for the dark mode palette.</param>
    /// <param name="paletteHighContrast">Optional override for the high-contrast palette.</param>
    /// <returns>
    /// A new <see cref="AllyariaPaletteVariant" /> instance with updated palettes and corresponding elevation hierarchies.
    /// </returns>
    public AllyariaPaletteVariant Cascade(AllyariaPalette? paletteLight = null,
        AllyariaPalette? paletteDark = null,
        AllyariaPalette? paletteHighContrast = null)
    {
        var newLight = paletteLight ?? LightPalette;
        var newDark = paletteDark ?? DarkPalette;
        var newHighContrast = paletteHighContrast ?? HighContrastPalette;

        return new AllyariaPaletteVariant(newLight, newDark, newHighContrast);
    }

    /// <summary>
    /// Returns the <see cref="AllyariaPalette" /> appropriate for the given theme type, elevation level, and component state.
    /// </summary>
    /// <param name="themeType">The selected theme type (light, dark, or high-contrast).</param>
    /// <param name="elevation">The component’s elevation level within the UI hierarchy.</param>
    /// <param name="state">The component’s visual state (e.g., default, hovered, focused).</param>
    /// <returns>The <see cref="AllyariaPalette" /> corresponding to the specified theme, elevation, and state.</returns>
    internal AllyariaPalette ToPalette(ThemeType themeType, ComponentElevation elevation, ComponentState state)
        => themeType switch
        {
            ThemeType.Dark => DarkElevation.ToPalette(elevation, state),
            ThemeType.HighContrast => HighContrastElevation.ToPalette(elevation, state),
            _ => LightElevation.ToPalette(elevation, state)
        };
}
