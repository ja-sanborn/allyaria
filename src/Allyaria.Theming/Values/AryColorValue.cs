...
public sealed class AryColorValue : ValueBase
{
    ...
    private static readonly Dictionary<string, AryColorValue> MaterialMap = new(StringComparer.OrdinalIgnoreCase)
    {
        // Red
        ["red50"] = Colors.Red50,
    ...
    private static readonly Dictionary<string, AryColorValue> WebNameMap = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    /// <exception cref="AryArgumentException">Thrown if any channel lies outside the valid range.</exception>
    private AryColorValue(double h, double s, double v, double a = 1.0)
        : base(string.Empty)
    {
        AryArgumentException.ThrowIfOutOfRange<double>(h, 0.0, 360.0, nameof(h));
        AryArgumentException.ThrowIfOutOfRange<double>(s, 0.0, 100.0, nameof(s));
        AryArgumentException.ThrowIfOutOfRange<double>(v, 0.0, 100.0, nameof(v));
        AryArgumentException.ThrowIfOutOfRange<double>(a, 0.0, 1.0, nameof(a));
    ...
    /// <param name="a">Alpha in [0–1].</param>
    /// <exception cref="AryArgumentException">Thrown if <paramref name="a" /> lies outside [0,1].</exception>
    ...
    /// </param>
    /// <exception cref="AryArgumentException">Thrown when the value cannot be parsed.</exception>
    public AryColorValue(string value)
        : base(string.Empty)
    {
        try
        {
            ...
            throw new AryArgumentException("Unable to parse value.", nameof(value), value);
        }
    ...
    private static void FromHexString(string s, out byte r, out byte g, out byte b, out double a)
    {
        ...
        throw new AryArgumentException($"Hex color must be #RGB, #RGBA, #RRGGBB, or #RRGGBBAA: '{s}'.", nameof(s), s);
    }
    ...
    /// <returns>The <see cref="AryColorValue" /> from the HSVA channels.</returns>
    public static AryColorValue FromHsva(double h, double s, double v, double a = 1.0)
    {
        var hh = h % 360.0;
        ...
        return new AryColorValue(hh, ss, vv, aa);
    }
    ...
    /// <param name="a">Alpha in [0..1], clamped.</param>
    /// <returns>The <see cref="AryColorValue" /> from the RGBA channels.</returns>
    public static AryColorValue FromRgba(byte r, byte g, byte b, double a = 1.0) => new(r, g, b, a);
    ...
    /// <exception cref="AryArgumentException">Thrown when the resulting value lies outside the provided range.</exception>
    private static double ParseDouble(string value, string param, int min, int max)
    {
        ...
        AryArgumentException.ThrowIfOutOfRange<double>(v, min, max, param);
    ...
    /// <exception cref="AryArgumentException">Thrown when <paramref name="c" /> is not a valid hex digit.</exception>
    private static int ToHexNibble(char c)
        => c switch
        {
            ...
            >= 'a' and <= 'f' => c - 'a' + 10,
            >= 'A' and <= 'F' => c - 'A' + 10,
            _ => throw new AryArgumentException($"Invalid hex digit '{c}'.")
        };
    ...
    private static bool TryFromMaterialName(string name, out AryColorValue color)
    {
    ...
    /// <param name="color">When this method returns, contains the parsed color if successful; otherwise the default value.</param>
    /// <returns><c>true</c> if parsing succeeded; otherwise <c>false</c>.</returns>
    private static bool TryFromWebName(string name, out AryColorValue color)
    {
    ...
    /// <summary>Implicit cast from <see cref="string" /> to <see cref="AryColorValue" />.</summary>
    /// <param name="value">The color value to parse.</param>
    /// <returns>A new <see cref="AryColorValue" />.</returns>
    public static implicit operator AryColorValue(string value) => new(value);

    /// <summary>
    /// Implicit cast from <see cref="AryColorValue" /> to its canonical string representation (<c>#RRGGBBAA</c>).
    /// </summary>
